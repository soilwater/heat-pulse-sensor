...
is 0.0001 mm; this is not a current-density or measurement-error simulation.
"""
...
def self_test():
    ...
    for mode, expected in [("clean", True), ("cross", False),
        ...
        if mode in ("cross", "annulus", "parallel_width_overlap", "inner_layer_annulus"):
            assert result["load_pads"] == ["LOAD.1"], (mode, result)
        print(f"PASS {mode}")
    print("Eight physical-copper Kelvin regression fixtures passed.")
