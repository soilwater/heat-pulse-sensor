...
VARIANT = HERE.parent
if VARIANT.name != "flat-nano":
    raise SystemExit("This exporter is restricted to the flat-nano project")
os.chdir(HERE)
TMP = VARIANT / "checks" / "tmp"
...
def build_assembly(stage):
    ...
    if board.GetCopperLayerCount() != 4:
        raise ValueError("The compact flat-nano requires four copper layers")
    ...
    for ref, part in parts.items():
        fp = actual[ref]
        if fp.GetValue() != part["value"] or not schematic_footprint_matches(part["footprint"], fp.GetFPIDAsString()):
    ...
    if len(rows) != len(expected) or {r["Ref"] for r in rows} != expected:
        raise ValueError("Front-side placement list must include every fitted flat-nano component exactly once")
...
def build_package(stage):
    """Write a complete fresh package, without changing the published release."""
    stage = Path(stage).resolve()
    if TMP.resolve() not in stage.parents or not stage.is_dir() or any(stage.iterdir()):
        raise ValueError("Package staging must be an empty directory under flat-nano/checks/tmp")
    build_cam(stage)
