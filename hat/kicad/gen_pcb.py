"""Compact face-to-face Nano R4 heat-pulse hat, r4 - PCB generator: outline, placement and prong wiring. route_body.py then routes the body and pours the ground plane.
...
if GAUGE != "12G":
    raise SystemExit("The compact hat preserves the current 12G sensor configuration only.")
ND = NEEDLES[GAUGE]
...
HEATER_PRONG_W = 1.4  # same heater prong as flat-nano: fits the 0603 heater body and insulation in the 12G bore.
...
ROOT_HW, FLARE_L = 1.2, 1.5                  # each prong flares to 2*ROOT_HW (2.4 mm) at the body junction, tapering to its own
                                             # width over FLARE_L mm. Buried in the epoxy head; answers JLC's >=2 mm tab.
HEATER_FIRST_Z = 8.8                         # heater, lanes and thermistor positions match flat-nano exactly
...
NOT_FITTED = {"J1", "J3", "J4"}     # omitted from factory assembly; solder wires/headers later
b = pcbnew.BOARD()
...
LABELS = [("12V", 3.8, -4), ("SDI", 3.8, 0), ("GND", 3.8, 4),
          ("HP-HAT", 21.5, 0), ("NANO FACES IN", 22, 2), ("USB", 2, -2)]
for s, x, y in LABELS:
...
def bore_margin(h):
    """Clearance between the top of a soldered part (h mm tall incl. solder, 0.5 mm wide) and the needle bore, board resting on the bore."""
    r = ND["bore"] / 2; yb = -math.sqrt(r * r - PRONG_W ** 2 / 4)
    return math.sqrt(r * r - 0.0625) - (yb + BOARD_T + h)


out = "hp_hat.kicad_pcb" if GAUGE == "12G" else f"hp_sensor_{GAUGE}.kicad_pcb"
pcbnew.SaveBoard(out, b)
from configure_fabrication import configure_file
configure_file(out)
print(f"{out}: {GAUGE} needle (OD {ND['od']}, bore {ND['bore']}) | prong {PRONG_W} x {BOARD_T} mm, bore margin over resistors {bore_margin(0.40):.2f} / thermistor {bore_margin(0.55):.2f} mm")
